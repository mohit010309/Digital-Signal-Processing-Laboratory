...
N = 8; % Size of the seqeunce x[n]
x = randperm(10,N); % using randperm(k,N) to select RANDOM N integers from range 1-k , here it selects N=8 integers from range 1-10
...
xlabel('samples(k) ->');
ylabel('X(k) ->');
title('8-point DFT of sequence x[n] using USER-DEFINED function my\_dit\_fft(x,N)');
grid on;
